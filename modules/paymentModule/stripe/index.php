<?php
    $proccessorResult = mysqli_query($con, "SELECT * FROM caliweb_paymentconfig WHERE processorName = 'Stripe'");
    $proccessorInfo = mysqli_fetch_array($proccessorResult);
    mysqli_free_result($proccessorResult);

    $paymentProccessorStatus = $proccessorInfo['status'];

    if ($paymentProccessorStatus == "Active") {
            ...
            if (mysqli_num_rows($stripeIDResult) > 0) {
                ...
                while ($stripeIDRow = mysqli_fetch_assoc($stripeIDResult)) {
                    ...
                    foreach ($paymentMethods->data as $paymentMethod) {
                        ...
                        echo '<td style="width:25%;"><span class="display-flex align-center">';
